...
// TempVoice returns the definition for the /tempvoice command.
	...
	return &Command{
		Definition: &discordgo.ApplicationCommand{
			...
			Options: []*discordgo.ApplicationCommandOption{
				{
					Name:        "setup",
					Description: "Configure temporary voice channels (Admin only)",
		...
		Handler: func(s *discordgo.Session, i *discordgo.InteractionCreate) {
			...
			if subcommand.Name == "setup" {
				...
				err := database.SetTempVoiceConfig(context.Background(), i.GuildID, categoryID, triggerChannelID)
				if err != nil {
					SendError(s, i, "Failed to configure temporary voice channels: "+err.Error())
					return
				}

				embed := &discordgo.MessageEmbed{
					Title:       "🎤 Temporary Voice Configured",
					Description: fmt.Sprintf("Users joining <#%s> will now have a temporary voice channel created for them in the selected category.", triggerChannelID),
					Color:       0x00FF00,
				}
				SendEmbed(s, i, embed)
			}
